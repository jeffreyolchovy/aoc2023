...
do
  ...
  v_results=$(sort -n <(for n in ${v_cols[@]}; do echo $n; done) | uniq -c | sort -nr | head -n 1 | awk '{$1=$1;print}')
  if [[ ${#xs[@]} == $(echo $v_results | cut -f 1 -d ' ') ]]
  then
  ...
  h_results=$(sort -n <(for n in ${h_cols[@]}; do echo $n; done) | uniq -c | sort -nr | head -n 1 | awk '{$1=$1;print}')
  if [[ ${#ys[@]} == $(echo $h_results | cut -f 1 -d ' ') ]]
  then
